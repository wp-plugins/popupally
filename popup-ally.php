...
if (!class_exists('PopupAlly')) {
	class PopupAlly {
		...
		private static function generate_popup_html($id, &$setting) {
			...
			$hidden_fields = '';
			foreach ($setting['hidden-form-fields'] as $name => $value) {
				$hidden_fields .= '<input type="hidden" name="' . $name . '" value="' . esc_attr($value) . '"/>';
			}
			foreach ($setting['other-form-fields'] as $name => $value) {
